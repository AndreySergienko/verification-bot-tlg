...
export class BotService implements OnModuleInit {
  ...
  async startBot() {
      global.bot.on('chat_join_request', async ({ user_chat_id, chat, date } : TelegramBot.ChatJoinRequest) => {
          try {
              const userId = user_chat_id
              const isInclude = await this.botRepository.findOne({ where: { userId } })
              if (isInclude) return
              const chatId = chat.id
              await this.botRepository.create({ userId, date, chatId })
              await global.bot.sendMessage(userId, validateMsg,  useSendMessage({
      ...
      global.bot.on('callback_query', async ({ from }: TelegramBot.CallbackQuery) => {
          try {
